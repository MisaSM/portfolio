...
- **Hero:** greeting, name, role, summary, then a `dl` of facts (Currently, Based in, Education, Languages) with hairline rows. The anemone sits to the right on desktop (top-right and cropped off the edge on mobile). On desktop its stem runs down and ends exactly on the hairline that opens the next section. On mobile, where the stem would pass behind the hero text, it fades out smoothly just below the bract collar, above the greeting.
